...
@interface VideoDetailsVC ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (yOffset < -64) {
        ...
        self.navigationItem.title = @"店铺名字";
        [[NSNotificationCenter defaultCenter] postNotificationName:@"goTop" object:nil userInfo:@{@"canScroll":@"1"}];
        [self.navigationController.navigationBar setBackgroundImage:nil forBarPosition:UIBarPositionAny barMetrics:UIBarMetricsDefault];
        [self.navigationController.navigationBar setShadowImage:nil];
    }
...
-(UIView *)setPageViewControllers {
    ...
    if (!_RCSegView) {
        videoIntroduceVC * First=[[videoIntroduceVC alloc]init];
        videoDirectoryVC * Third=[[videoDirectoryVC alloc]init];
        RelatedVideoVC * Second = [[RelatedVideoVC alloc] init];
        NSArray *controllers=@[First,Third,Second];
        NSArray *titleArray =@[@"项目",@"目录", @"相关视频"];
