...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        _payBtn.backgroundColor = kNavigation_Color;
        _payBtn.y = 0;
        [_payBtn addTarget:self action:@selector(handleSettlement:) forControlEvents:(UIControlEventTouchUpInside)];
        ...
        [attStr addAttribute:NSForegroundColorAttributeName value:kNavigation_Color range:range1];
...
- (void)selectedClick:(UIButton *)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(bottomPriceView:)]) {
        [self.delegate bottomPriceView:sender.selected];
    }
}
...
- (void)handleSettlement:(UIButton *)sender {
    if ([_delegate respondsToSelector:@selector(SettlementClick:)]) {
        [_delegate SettlementClick:sender];
    }
}
